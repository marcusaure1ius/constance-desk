...
describe("карточка ответа — экранируемые символы при бюджете на исходе", () => {
  ...
  /** Она же, собранная по числу скрытых задач: длину ей задаёт склонение. */
  const hiddenLineFor = (hidden: number) =>
    `\n…и ещё ${hidden} ${plural(hidden, TASK_FORMS)} — все на доске`;

  /**
   * Сколько места карточка занимает под «…и ещё N», ещё не зная N: по общему
   * числу задач и по самой длинной форме склонения.
   */
  const reserveFor = (total: number) =>
    `\n…и ещё ${total} ${TASK_FORMS.reduce((a, b) => (b.length > a.length ? b : a))} — все на доске`
      .length;

  /** Задач в тяжёлом наполнении. Число не случайное — почему именно столько, ниже. */
  const HEAVY_TASKS = 34;
   ...
   *
   * Задач именно тридцать четыре, и число не случайное. Место под «…и ещё N»
   * занимается заранее — по общему числу задач и по самой длинной форме
   * склонения, потому что N в тот момент ещё неизвестно. Мутант, ошибающийся
   * на символ, показывает на задачу больше: скрытых становится на одну меньше,
   * строка про них пересобирается, и наружу лишний символ выходит только тогда,
   * когда укоротившееся число со своим склонением всё ещё занимает ровно
   * зарезервированное место.
   *
   * При тридцати четырёх это так: карточка стоит в 4096 со строкой «…и ещё 22
   * задачи», мутант печатает «…и ещё 21 задача» той же длины и вылезает до 4097
   * (18 случаев на прогоне, все до единого — с этой строкой). При тридцати
   * трёх — уже нет: «…и ещё 21 задача» ужимается до «…и ещё 20 задач», символ
   * уходит на склонение, превышение съедается, прогон под мутантом зелёный.
   * При двадцати пяти и двадцати двух укоротившаяся строка до запаса не
   * дотягивает ни разу — мутант живёт; при тридцати пяти снова гибнет
   * (24 случая).
   */
  const fills: { what: string; tasks: CreatedTask[]; others: CapturedItem[] }[] = [
    ...
    ...Array.from({ length: 25 }, (_, i) => ({
      what: `${HEAVY_TASKS} задачи с эпиком и сроком, заголовок в ${176 + i} символов`,
      tasks: Array.from({ length: HEAVY_TASKS }, (_, n) => ({
  ...
  it("перебор объёма карточки: остаток под расшифровку проходит окно от 600 до нуля", () => {
    ...
    /**
     * Наполнения, на которых лишнему символу нечем закрыться: карточка ровно у
     * лимита, а строка про N−1 скрытых задач — та, что напечаталась бы, покажи
     * карточка на задачу больше, — заняла бы ровно зарезервированное место.
     */
    const unforgiving = new Set<number>();
    ...
    for (const fill of fills) {
      for (let name = 1; name <= 64; name++) {
        ...
        for (const [shape, char] of Object.entries(shapes)) {
          ...
          const hidden = Number(card.text.match(HIDDEN_TASKS_LINE)?.[1]);
          if (
            card.text.length === TELEGRAM_MESSAGE_LIMIT &&
            hiddenLineFor(hidden - 1).length === reserveFor(fill.tasks.length)
          ) {
            unforgiving.add(fill.tasks.length);
          }
    ...
    // Состав наполнений — не деталь оформления: выброси отсюда тяжёлые случаи,
    // и мутант «add ошибается на символ» воскресает, а перебор молча остаётся
    // зелёным. Стеречь их «карточка у потолка» мало: при тридцати трёх задачах
    // потолок берётся, а мутант выживает. Сторожить надо ту единственную
    // клетку, где лишний символ не съедается ни запасом, ни склонением.
    // Прогон: 34 задачи — 309 таких случаев (мутант гибнет на 18 из них),
    // 35 — 1871, а 33, 25 и 22 не дают ни одного, и мутант на них живёт.
    expect(
      [...unforgiving],
      "наполнения, где лишнему символу нечем закрыться"
    ).toContain(HEAVY_TASKS);
  });
