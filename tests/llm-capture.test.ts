...
describe("sanitizeTitle", () => {
  it("снимает «надо» и «нужно бы» в начале и поднимает обнажившуюся букву", () => {
    // Живая проверка показала обрывок на доске: «Надо написать комментарии»
    // превращалось в задачу «написать комментарии по стратегии».
  ...
  it("регистр автора не трогает, если начало не снимали", () => {
    expect(sanitizeTitle("написать Юре по альфа таргету")).toBe(
      "написать Юре по альфа таргету"
    );
    expect(sanitizeTitle("mcp ms exchange протестировать")).toBe(
      "mcp ms exchange протестировать"
    );
  });

  it("поднимает букву и когда после «надо» идёт латиница или уже заглавная", () => {
    expect(sanitizeTitle("Надо mcp протестировать")).toBe("Mcp протестировать");
    expect(sanitizeTitle("Надо Юре написать")).toBe("Юре написать");
  });
  ...
  it("упоминание вплотную после дефиса тоже снимается", () => {
    // Дефис стоял в списке «символов почты» перед «собакой», и упоминание после
    // тире не снималось вовсе: в заголовке оставалось «@ivan_petrov».
    expect(sanitizeTitle("созвон -@ivan_petrov завтра")).toBe("созвон - завтра");
    expect(sanitizeTitle("слайды —@ivan_petrov до среды")).toBe("слайды — до среды");
  });

  it("адрес с дефисом внутри остаётся целым", () => {
    expect(sanitizeTitle("написать на ivan-petrov@company.com про кку")).toBe(
      "написать на ivan-petrov@company.com про кку"
    );
  });

  it("упоминание перед знаком препинания не оставляет пробел", () => {
    // «Спросить @ivan_petrov, потом позвонить» превращалось в «Спросить , потом».
    expect(sanitizeTitle("Спросить @ivan_petrov, потом позвонить")).toBe(
      "Спросить, потом позвонить"
    );
    expect(sanitizeTitle("Позвонить @ivan_petrov.")).toBe("Позвонить.");
    expect(sanitizeTitle("Успеть до среды @ivan_petrov!")).toBe("Успеть до среды!");
    // Пробел, который поставил автор, остаётся его пробелом.
    expect(sanitizeTitle("Спросить у коллег , потом позвонить")).toBe(
      "Спросить у коллег , потом позвонить"
    );
  });
